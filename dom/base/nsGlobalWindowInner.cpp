...
nsGlobalWindowInner::nsGlobalWindowInner(nsGlobalWindowOuter* aOuterWindow)
  ...
  if (aOuterWindow) {
    ...
    mTimeoutManager = MakeUnique<mozilla::dom::TimeoutManager>(
        *nsGlobalWindowInner::Cast(AsInner()));
...
void nsGlobalWindowInner::CleanUp() {
...
NS_INTERFACE_MAP_BEGIN_CYCLE_COLLECTION(nsGlobalWindowInner)
  ...
  } else if (aIID.Equals(NS_GET_IID(mozIDOMWindow))) {
...
void nsGlobalWindowInner::SetOpenerWindow(nsPIDOMWindowOuter* aOpener,
...
EventTarget* nsGlobalWindowInner::GetTargetForEventTargetChain() {
  return this;
}

nsresult nsGlobalWindowInner::WillHandleEvent(EventChainPostVisitor& aVisitor) {
  return NS_OK;
}

nsresult nsGlobalWindowInner::GetEventTargetParent(
...
void nsGlobalWindowInner::DisableDialogs() {
  FORWARD_TO_OUTER_VOID(DisableDialogs, ());
}

void nsGlobalWindowInner::EnableDialogs() {
...
nsIDOMScreen* nsGlobalWindowInner::GetScreen() {
  ErrorResult dummy;
...
void nsPIDOMWindowInner::Freeze() { nsGlobalWindowInner::Cast(this)->Freeze(); }

void nsPIDOMWindowInner::Thaw() { nsGlobalWindowInner::Cast(this)->Thaw(); }
...
nsPIDOMWindowInner::GetOrCreateServiceWorker(
    const mozilla::dom::ServiceWorkerDescriptor& aDescriptor) {
  return Move(
      nsGlobalWindowInner::Cast(this)->GetOrCreateServiceWorker(aDescriptor));
}
...
bool nsPIDOMWindowInner::GetAudioCaptured() const { return mAudioCaptured; }

nsresult nsPIDOMWindowInner::SetAudioCapture(bool aCapture) {
 ...
 */
nsPIDOMWindowOuter* nsGlobalWindowInner::GetScriptableParentOrNull() {
...
already_AddRefed<nsIDOMWindowCollection> nsGlobalWindowInner::GetFrames() {
  ...
  if (nameSpaceManager) {
    ...
    for (auto i = nameSpaceManager->GlobalNameIter(); !i.Done(); i.Next()) {
      ...
      if (nsWindowSH::NameStructEnabled(aCx, this, entry->mKey,
        ...
        JSString* str = JS_AtomizeUCStringN(aCx, entry->mKey.BeginReading(),
                                            entry->mKey.Length());
...
nsIDOMOfflineResourceList* nsGlobalWindowInner::GetApplicationCache(
    ErrorResult& aError) {
  if (!mApplicationCache) {
...
void nsGlobalWindowInner::RefreshCompartmentPrincipal() {
  JS_SetCompartmentPrincipals(
      js::GetObjectCompartment(GetWrapperPreserveColor()),
...
void nsGlobalWindowInner::Back(ErrorResult& aError) {
  FORWARD_TO_OUTER_OR_THROW(BackOuter, (aError), aError, );
}

void nsGlobalWindowInner::Forward(ErrorResult& aError) {
  FORWARD_TO_OUTER_OR_THROW(ForwardOuter, (aError), aError, );
}

void nsGlobalWindowInner::Home(nsIPrincipal& aSubjectPrincipal,
                               ErrorResult& aError) {
...
void nsGlobalWindowInner::ScrollTo(const CSSIntPoint& aScroll,
  ...
  if (sf) {
    ...
    bool smoothScroll =
        sf->GetScrollbarStyles().IsSmoothScroll(aOptions.mBehavior);
...
void nsGlobalWindowInner::ScrollBy(double aXScrollDif, double aYScrollDif) {
  ...
  if (sf) {
    // Convert -Inf, Inf, and NaN to 0; otherwise, convert by C-style cast.
    auto scrollDif =
        CSSIntPoint::Truncate(mozilla::ToZeroIfNonfinite(aXScrollDif),
...
nsGlobalWindowInner* nsGlobalWindowInner::CallerInnerWindow() {
...
void nsGlobalWindowInner::ReallyCloseWindow() {
 ...
 */
already_AddRefed<nsIDOMElement> nsGlobalWindowInner::GetFrameElement() {
  ErrorResult dummy;
...
/* static */ bool nsGlobalWindowInner::TokenizeDialogOptions(
    nsAString& aToken, nsAString::const_iterator& aIter,
  ...
  while (aIter != aEnd && !nsCRT::IsAsciiSpace(*aIter) && *aIter != ';' &&
         *aIter != ':' && *aIter != '=') {
    ++aIter;
  }
...
/* static */
void nsGlobalWindowInner::ConvertDialogOptions(const nsAString& aOptions,
                                               nsAString& aResult) {
  ...
  while (true) {
    ...
    // Invalid name.
    if (name.EqualsLiteral("=") || name.EqualsLiteral(":") ||
        name.EqualsLiteral(";")) {
      break;
    }
    ...
    if (iter == end || !TokenizeDialogOptions(token, iter, end) ||
        !token.EqualsLiteral(";")) {
      break;
    }
  }
...
void nsGlobalWindowInner::AddEventListener(
...
nsIScriptContext* nsGlobalWindowInner::GetContextForEventHandlers(
    nsresult* aRv) {
...
nsPIDOMWindowOuter* nsGlobalWindowInner::GetPrivateRoot() {
  nsGlobalWindowOuter* outer = GetOuterWindowInternal();
  if (!outer) {
    NS_WARNING("No outer window available!");
    return nullptr;
...
bool nsGlobalWindowInner::IsTopLevelWindowActive() {
  if (GetOuterWindowInternal()) {
...
void nsGlobalWindowInner::SetFocusedNode(nsIContent* aNode,
...
nsresult nsGlobalWindowInner::GetComputedStyleHelper(
    nsIDOMElement* aElt, const nsAString& aPseudoElt, bool aDefaultStylesOnly,
...
void nsGlobalWindowInner::AddPendingPromise(mozilla::dom::Promise* aPromise) {
  mPendingPromises.AppendElement(aPromise);
}

void nsGlobalWindowInner::RemovePendingPromise(
    mozilla::dom::Promise* aPromise) {
  DebugOnly<bool> foundIt = mPendingPromises.RemoveElement(aPromise);
  MOZ_ASSERT(foundIt,
             "tried to remove a non-existent element from mPendingPromises");
}
...
already_AddRefed<CacheStorage> nsGlobalWindowInner::GetCaches(
  ...
  if (!mCacheStorage) {
    ...
    // We don't block the cache API when being told to only allow storage for
    // the current session.
    bool storageBlocked =
        access <= nsContentUtils::StorageAccess::ePrivateBrowsing;

    mCacheStorage = CacheStorage::CreateOnMainThread(
        cache::DEFAULT_NAMESPACE, this, GetPrincipal(), storageBlocked,
...
class NotifyIdleObserverRunnable : public Runnable {
 public:
  NotifyIdleObserverRunnable(nsIIdleObserver* aIdleObserver, uint32_t aTimeInS,
...
bool nsGlobalWindowInner::ContainsIdleObserver(nsIIdleObserver* aIdleObserver,
                                               uint32_t aTimeInS) {
...
nsresult nsGlobalWindowInner::FindIndexOfElementToRemove(
    nsIIdleObserver* aIdleObserver, int32_t* aRemoveElementIndex) {
  MOZ_ASSERT(aIdleObserver, "Idle observer not instantiated.");
...
static bool IsInterval(const Optional<int32_t>& aTimeout,
...
int32_t nsGlobalWindowInner::SetInterval(
    ...
    const Optional<int32_t>& aTimeout, const Sequence<JS::Value>& /* unused */,
    ErrorResult& aError) {
  int32_t timeout;
  bool isInterval = IsInterval(aTimeout, timeout);
...
nsIMessageBroadcaster* nsGlobalWindowInner::GetMessageManager(
    ErrorResult& aError) {
  MOZ_ASSERT(IsChromeWindow());
  if (!mChromeFields.mMessageManager) {
    nsCOMPtr<nsIMessageBroadcaster> globalMM =
        do_GetService("@mozilla.org/globalmessagemanager;1");
    mChromeFields.mMessageManager = new nsFrameMessageManager(
        nullptr, static_cast<nsFrameMessageManager*>(globalMM.get()),
...
nsIMessageBroadcaster* nsGlobalWindowInner::GetGroupMessageManager(
    const nsAString& aGroup, ErrorResult& aError) {
  MOZ_ASSERT(IsChromeWindow());
  ...
  nsCOMPtr<nsIMessageBroadcaster> messageManager =
      mChromeFields.mGroupMessageManagers.LookupForAdd(aGroup).OrInsert(
          [this, &aError]() {
            ...
            return new nsFrameMessageManager(nullptr, parent,
                                             MM_CHROME | MM_BROADCASTER);
          });
...
already_AddRefed<mozilla::dom::Promise> nsGlobalWindowInner::CreateImageBitmap(
    JSContext* aCx, const ImageBitmapSource& aImage, int32_t aOffset,
    int32_t aLength, ImageBitmapFormat aFormat,
    const Sequence<ChannelPixelLayout>& aLayout, ErrorResult& aRv) {
...
Worklet* nsGlobalWindowInner::GetAudioWorklet(ErrorResult& aRv) {
  if (!mAudioWorklet) {
    nsIPrincipal* principal = GetPrincipal();
    if (!principal) {
