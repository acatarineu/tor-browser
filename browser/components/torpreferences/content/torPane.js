...
} = ChromeUtils.import("chrome://browser/content/torpreferences/torBridgeSettings.jsm");
...
} = ChromeUtils.import("chrome://browser/content/torpreferences/torProxySettings.jsm");
const {
  TorFirewallSettings,
  makeTorFirewallSettingsNone,
  makeTorFirewallSettingsCustom,
} = ChromeUtils.import("chrome://browser/content/torpreferences/torFirewallSettings.jsm");

const {
  TorLogDialog
} = ChromeUtils.import("chrome://browser/content/torpreferences/torLogDialog.jsm");

const {
  RequestBridgeDialog
} = ChromeUtils.import("chrome://browser/content/torpreferences/requestBridgeDialog.jsm");
...
const {
  ...
  parseBridgeStrings,
  parsePortList,
} = ChromeUtils.import("chrome://browser/content/torpreferences/parseFunctions.jsm");
...
const gTorPane = function()
{
  ...
  const selectors = {
    ...
    bridges : {
      ...
      bridgeSelectionRadiogroup : "radiogroup#torPreferences-bridges-bridgeSelection",
      ...
      requestBridgeTextarea : "textarea#torPreferences-bridges-textareaRequestBridge",
      provideBridgeOption : "radio#torPreferences-bridges-radioProvideBridge",
      provideBridgeDescription : "description#torPreferences-bridges-descriptionProvideBridge",
      provideBridgeTextarea : "textarea#torPreferences-bridges-textareaProvideBridge",
    },
    advanced : {
      ...
      firewallAllowedPortsTextbox : "textbox#torPreferences-advanced-textboxAllowedPorts",
  ...
  let retval = {
    ...
    _populateXUL : function() {
      ...
      document.querySelector(selectors.category.title).setAttribute("value", TorStrings.settings.categoryTitle);

      let prefpane = document.getElementById("mainPrefPane");

      // Heading
      prefpane.querySelector(selectors.torPreferences.header).innerText = TorStrings.settings.torPreferencesHeading;
      prefpane.querySelector(selectors.torPreferences.description).textContent = TorStrings.settings.torPreferencesDescription;
      {
        let learnMore = prefpane.querySelector(selectors.torPreferences.learnMore);
        learnMore.setAttribute("value", TorStrings.settings.learnMore);
        learnMore.setAttribute("href", TorStrings.settings.learnMoreTorBrowserURL);
      }

      // Bridge setup
      prefpane.querySelector(selectors.bridges.header).innerText = TorStrings.settings.bridgesHeading;
      prefpane.querySelector(selectors.bridges.description).textContent = TorStrings.settings.bridgesDescription;
      {
      ...
      this._useBridgeCheckbox.setAttribute("label", TorStrings.settings.useBridge);
      ...
      this._builtinBridgeOption.setAttribute("label", TorStrings.settings.selectBridge);
      ...
      this._requestBridgeOption.setAttribute("label", TorStrings.settings.requestBridgeFromTorProject);
      ...
      this._requestBridgeButton.setAttribute("label", TorStrings.settings.requestNewBridge);
      ...
      this._provideBridgeOption.setAttribute("label", TorStrings.settings.provideBridge);
      this._provideBridgeOption.setAttribute("value", TorBridgeSource.USERPROVIDED);
      prefpane.querySelector(selectors.bridges.provideBridgeDescription).textContent = TorStrings.settings.provideBridgeDirections;
      this._provideBridgeTextarea = prefpane.querySelector(selectors.bridges.provideBridgeTextarea)
      this._provideBridgeTextarea.setAttribute("placeholder", TorStrings.settings.provideBridgePlaceholder);

      // Advanced setup
      prefpane.querySelector(selectors.advanced.header).innerText = TorStrings.settings.advancedHeading;
      prefpane.querySelector(selectors.advanced.description).textContent = TorStrings.settings.advancedDescription;
      {
        let learnMore = prefpane.querySelector(selectors.advanced.learnMore);
        learnMore.setAttribute("value", TorStrings.settings.learnMore);
        learnMore.setAttribute("href", TorStrings.settings.learnMoreNetworkSettingsURL);
      }

      // Local Proxy
      this._useProxyCheckbox = prefpane.querySelector(selectors.advanced.useProxyCheckbox);
      this._useProxyCheckbox.setAttribute("label", TorStrings.settings.useLocalProxy);
      ...
      let mockProxies = [
        { value : TorProxyType.SOCKS4, label : TorStrings.settings.proxyTypeSOCKS4 },
        { value : TorProxyType.SOCKS5, label : TorStrings.settings.proxyTypeSOCKS5 },
      ...
      this._proxyAddressLabel.setAttribute("value", TorStrings.settings.proxyAddress);
      this._proxyAddressTextbox = prefpane.querySelector(selectors.advanced.proxyAddressTextbox);
      this._proxyAddressTextbox.setAttribute("placeholder", TorStrings.settings.proxyAddressPlaceholder);
      ...
      this._proxyUsernameLabel.setAttribute("value", TorStrings.settings.proxyUsername);
      this._proxyUsernameTextbox = prefpane.querySelector(selectors.advanced.proxyUsernameTextbox);
      this._proxyUsernameTextbox.setAttribute("placeholder", TorStrings.settings.proxyUsernamePasswordPlaceholder);
      this._proxyPasswordLabel = prefpane.querySelector(selectors.advanced.proxyPasswordLabel);
      this._proxyPasswordLabel.setAttribute("value", TorStrings.settings.proxyPassword);
      this._proxyPasswordTextbox = prefpane.querySelector(selectors.advanced.proxyPasswordTextbox);
      this._proxyPasswordTextbox.setAttribute("placeholder", TorStrings.settings.proxyUsernamePasswordPlaceholder);

      // Local firewall
      this._useFirewallCheckbox = prefpane.querySelector(selectors.advanced.useFirewallCheckbox);
      this._useFirewallCheckbox.setAttribute("label", TorStrings.settings.useFirewall);
      this._allowedPortsLabel = prefpane.querySelector(selectors.advanced.firewallAllowedPortsLabel);
      this._allowedPortsLabel.setAttribute("value", TorStrings.settings.allowedPorts);
      this._allowedPortsTextbox = prefpane.querySelector(selectors.advanced.firewallAllowedPortsTextbox);
      this._allowedPortsTextbox.setAttribute("placeholder", TorStrings.settings.allowedPortsPlaceholder);

      // Tor logs
      prefpane.querySelector(selectors.advanced.torLogsLabel).setAttribute("value", TorStrings.settings.showTorDaemonLogs);
      prefpane.querySelector(selectors.advanced.torLogsButton).setAttribute("label", TorStrings.settings.showLogs);
    ...
    onRequestBridge : function() {
      ...
      requestBridgeDialog.openDialog(gSubDialog, self._proxySettings.proxyURI, aBridges => {
    ...
    onUpdateBridgeSettings : function() {
      ...
      switch(source) {
        ...
        case TorBridgeSource.USERPROVIDED:
        {
          ...
          if (bridgeStrings) {
            let bridgeStringList = parseBridgeStrings(bridgeStrings);
            bridgeSettings = makeTorBridgeSettingsUserProvided(bridgeStringList);
          } else {
            bridgeSettings = makeTorBridgeSettingsNone();
          }
          break;
    ...
    onUpdateProxySettings : function() {
      ...
      switch (proxyType) {
        case TorProxyType.NONE:
          ...
          break;
        case TorProxyType.SOCKS4:
          proxySettings = makeTorProxySettingsSocks4(addressString, parsePort(portString));
          break;
        case TorProxyType.SOCKS5:
          proxySettings = makeTorProxySettingsSocks5(addressString, parsePort(portString), usernameString, passwordString);
          break;
        case TorProxyType.HTTPS:
          proxySettings = makeTorProxySettingsHTTPS(addressString, parsePort(portString), usernameString, passwordString);
          break;
      }
    ...
    onUpdateFirewallSettings : function() {
      ...
      if (portListString) {
        firewallSettings = makeTorFirewallSettingsCustom(parsePortList(portListString));
      } else {
