...
class TorBridgeSettings {
  ...
  static get defaultBridgeTypes() {
    ...
    let bridgeListBranch = Services.prefs.getBranch(TorStrings.preferenceBranches.defaultBridge);
    ...
    let recommendedBridgeType = null;
    try {
      recommendedBridgeType = Services.prefs.getCharPref(TorStrings.preferenceKeys.recommendedBridgeType);
    } catch (e) { }
  ...
  _readDefaultBridges(aBridgeType) {
    let bridgeBranch = Services.prefs.getBranch(TorStrings.preferenceBranches.defaultBridge);
  ...
  _readBridgeDBBridges() {
    let bridgeBranch = Services.prefs.getBranch(`${TorStrings.preferenceBranches.bridgeDBBridges}`);
    let bridgeBranchPrefs = bridgeBranch.getChildList("", {});
  ...
  _readTorrcBridges() {
    let bridgeList = TorProtocolService.readStringArraySetting(TorStrings.configKeys.bridgeList);
  ...
  readSettings() {
    ...
    try {
      defaultBridgeType = Services.prefs.getCharPref(TorStrings.preferenceKeys.defaultBridgeType);
    } catch (e) { }
  ...
  writeSettings() {
    ...
    for(const pref of bridgeBranchPrefs) {
      Services.prefs.clearUserPref(`${TorStrings.preferenceBranches.bridgeDBBridges}${pref}`);
    }
    ...
    switch(this._bridgeSource) {
      case TorBridgeSource.BUILTIN:
        ...
        Services.prefs.setCharPref(TorStrings.preferenceKeys.defaultBridgeType, this._selectedDefaultBridgeType);
        break;
      case TorBridgeSource.BRIDGEDB:
        // save bridges off to prefs
        for(let i = 0; i < this.bridgeStringsArray.length; ++i) {
          Services.prefs.setCharPref(`${TorStrings.preferenceBranches.bridgeDBBridges}${i}`, this.bridgeStringsArray[i]);
        }
        break;
    }
    ...
    if (this._bridgeSource != TorBridgeSource.NONE) {
      ...
      settingsObject.set(TorStrings.configKeys.bridgeList, this.bridgeStringsArray);
    }
