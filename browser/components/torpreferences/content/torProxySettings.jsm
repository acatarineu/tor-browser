...
class TorProxySettings {
  ...
  readSettings()
  {
    let tlps = TorProtocolService._tlps;
    let reply;

    // SOCKS4
    {
      let addressPort = TorProtocolService.readStringSetting(TorStrings.configKeys.socks4Proxy);
      if (addressPort) {
        // address+port
    ...
    // SOCKS5
    {
      let addressPort = TorProtocolService.readStringSetting(TorStrings.configKeys.socks5Proxy);

      if (addressPort) {
        // address+port
        let [proxyAddress, proxyPort] = parseAddrPort(addressPort);
        // username
        let proxyUsername = TorProtocolService.readStringSetting(TorStrings.configKeys.socks5ProxyUsername);;
        // password
        let proxyPassword = TorProtocolService.readStringSetting(TorStrings.configKeys.socks5ProxyPassword);;
    ...
    // HTTP
    {
      let addressPort = TorProtocolService.readStringSetting(TorStrings.configKeys.httpsProxy);
      ...
      if (addressPort) {
        ...
        // username:password
        let proxyAuthenticator = TorProtocolService.readStringSetting(TorStrings.configKeys.httpsProxyAuthenticator);

        let [proxyUsername, proxyPassword] = ["", ""];
        if (proxyAuthenticator) {
          [proxyUsername, proxyPassword] = parseUsernamePassword(proxyAuthenticator);
        }
  ...
  writeSettings() {
    ...
    switch (this._proxyType) {
      case TorProxyType.SOCKS4:
        settingsObject.set(TorStrings.configKeys.socks4Proxy, `${this._proxyAddress}:${this._proxyPort}`);
        break;
      case TorProxyType.SOCKS5:
        settingsObject.set(TorStrings.configKeys.socks5Proxy, `${this._proxyAddress}:${this._proxyPort}`);
        settingsObject.set(TorStrings.configKeys.socks5ProxyUsername, this._proxyUsername);
        settingsObject.set(TorStrings.configKeys.socks5ProxyPassword, this._proxyPassword);
        break
      case TorProxyType.HTTPS:
        settingsObject.set(TorStrings.configKeys.httpsProxy, `${this._proxyAddress}:${this._proxyPort}`);
        settingsObject.set(TorStrings.configKeys.httpsProxyAuthenticator, `${this._proxyUsername}:${this._proxyPassword}`);
        break;
    }
...
function makeTorProxySettingsSocks5(aProxyAddress, aProxyPort, aProxyUsername, aProxyPassword) {
  let retval = new TorProxySettings();
...
function makeTorProxySettingsHTTPS(aProxyAddress, aProxyPort, aProxyUsername, aProxyPassword) {
  let retval = new TorProxySettings();
