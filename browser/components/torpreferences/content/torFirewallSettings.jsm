...
var EXPORTED_SYMBOLS = ["TorFirewallSettings", "makeTorFirewallSettingsNone", "makeTorFirewallSettingsCustom"];
...
class TorFirewallSettings {
  ...
  readSettings() {
    let addressPortList = TorProtocolService.readStringSetting(TorStrings.configKeys.reachableAddresses);
  ...
  writeSettings() {
    ...
    if (this._allowedPorts.length > 0) {
      settingsObject.set(TorStrings.configKeys.reachableAddresses, this.portsConfigurationString);
    }
