...
class TorLogDialog {
  ...
  _populateXUL(aDialog) {
    ...
    this._logTextarea = this._dialog.querySelector(TorLogDialog.selectors.logTextarea);

    this._copyLogButton = this._dialog.getButton(TorLogDialog.selectors.copyLogButton);
  ...
  init(window, aDialog) {
    ...
    window.setTimeout(function() {self._populateXUL(aDialog);}, 0);
  }
  ...
  openDialog(gSubDialog) {
    let self = this;
    gSubDialog.open("chrome://browser/content/torpreferences/torLogDialog.xul", "resizable=yes", self);
  }
};
