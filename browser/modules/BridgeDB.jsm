...
const { TorLauncherBridgeDB } = ChromeUtils.import("resource://torlauncher/modules/tl-bridgedb.jsm");
...
var BridgeDB = {
  ...
  submitCaptchaGuess : function(aCaptchaSolution) {
    ...
    if (this._moatRequestor && this._currentCaptchaInfo) {
      ...
      .then(aBridgeInfo =>
      {
        self._moatRequestor.close();
        self._moatRequestor = null;
        self._currentCaptchaInfo = null;
        self._bridges = aBridgeInfo.bridges;
  ...
  requestNewCaptchaImage : function (aProxyURI) {
    ...
    let transportPlugins = TorProtocolService.readStringArraySetting(TorStrings.configKeys.clientTransportPlugin);
    ...
    .then(() =>
    {
      ...
      return self._moatRequestor.fetchBridges([bridgeType]);
    })
    .then(aCaptchaInfo =>
    {
      // cache off the current captcha info as the challenge is needed for response
      self._currentCaptchaInfo = aCaptchaInfo;
