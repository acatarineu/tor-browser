...
var TorProtocolService = {
  ...
  _tlps : Cc["@torproject.org/torlauncher-protocol-service;1"].getService(Ci.nsISupports).wrappedJSObject,
  // maintain a map of tor settings set by Tor Browser so that we don't
  // repeatedly set the same key/values over and over
  // this map contains string keys to primitive or array values
  _settingsCache : new Map(),

  _typeof : function(aValue) {
    switch (typeof aValue) {
      case "boolean": return "boolean";
      case "string" : return "string";
      case "object":
        if (aValue == null) return "null";
        else if (Array.isArray(aValue)) return "array";
        return "object";
    }
    return "unknown";
  },

  _assertValidSettingKey : function(aSetting) {
    // ensure the 'key' is a string
    if ((typeof aSetting) != "string") {
      throw new Error(`Expected setting of type string but received ${typeof aSetting}`);
    }
  },

  _assertValidSetting : function(aSetting, aValue) {
      ...
      case "array":
        for(const element of aValue) {
          if ((typeof element) != "string") {
            throw new Error(`Setting '${aSetting}' array contains value of invalid type '${typeof element}'`);
          }
        }
        return;
      default:
        throw new Error(`Invalid object type received for setting '${aSetting}'`);
    }
  },

  // takes a Map containing tor settings
  // throws on error
  writeSettings : function(aSettingsObj) {
  ...
  _readSetting : function(aSetting) {
    this._assertValidSettingKey(aSetting);
  ...
  _readBoolSetting : function(aSetting) {
    let lineArray = this._readSetting(aSetting);
    if (lineArray.length != 1) {
      throw new Error (`Expected an array with length 1 but received array of length ${lineArray.length}`);
    }
    ...
    switch(retval) {
      case "0": return false;
      case "1": return true;
      default:  throw new Error(`Expected boolean (1 or 0) but received '${retval}'`);
    }
  },

  _readStringSetting : function(aSetting) {
    let lineArray = this._readSetting(aSetting);
    if (lineArray.length != 1) {
      throw new Error (`Expected an array with length 1 but received array of length ${lineArray.length}`);
    }
  ...
  _readStringArraySetting : function(aSetting) {
    let lineArray = this._readSetting(aSetting);
    return lineArray;
  },

  readBoolSetting : function(aSetting) {
    let value = this._readBoolSetting(aSetting);
    this._settingsCache.set(aSetting, value);
    return value;
  },

  readStringSetting : function(aSetting) {
    let value = this._readStringSetting(aSetting);
    this._settingsCache.set(aSetting, value);
    return value;
  },

  readStringArraySetting : function(aSetting) {
